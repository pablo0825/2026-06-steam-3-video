...
- 寬高數字以 `YELLOW` 或 `BLUE` 強調，避免整張卡只剩深色文字。

動畫：

- 標題淡入或 spring 進場。
- 左右尺寸卡由左右滑入。
- `1920` / `1080` 數字可用黃色底線或小 highlight bar 依序點亮。
- 段尾淡出銜接 S07 的圖片滿版。
...
- 底部置中小字標示 `1080×1920`。
- 尺寸標籤同 S07，使用白字與半透明深色底；若黑底上可讀性足夠，也可只用白字。
...
- S07 圖片必須滿版覆蓋整個 1920×1080，底部標示 `1920×1080`。
- S08 背景必須為黑色，直式圖片完整置中，不裁切，底部標示 `1080×1920`。
